...
function [Dinit,Tinit,Winit,Q,Xtemp,dictLabel]=initialization4LCKSVD(training_feats,H_train,dictsize,iterations,sparsitythres)

numClass = size(H_train,1); % 有多少类
numPerClass = round(dictsize/numClass); % initial points from each classes向上取整
Dinit = []; % for LC-Ksvd1 and LC-Ksvd2
dictLabel = [];
for classid=1:numClass
    col_ids = find(H_train(classid,:)==1);%某一行的哪几列是1
